 ...
 SubTotalChange() {
  ...
  onDeleteRow(rowIndex)
  {
   ...
   async Delete()
    {
       let editproduct=this.form.get('product') as FormArray;
      await  this.api.get('delete_data.php?authToken='+environment.authToken+'&table=credit_note_item&field=id&id='+this.item_DeleteId).then( async(data: any) =>
        {
                      ...
                      table: {
                        ...
                        body: [
                          [
                            [
                              [
                                { text: ' CREDIT NOTE', fontSize: 11, bold: true,border: [false, false, false, false], alignment: 'center' },]
                            ],
                          ],
                        ]
                      },
...
async  onSubmit(value)
  ...
  {
      this.loading = true
        await this.api.get('mp_delete_invoice.php?table=credit_note&field=id&item_field=credit_note_id&delete_id='+this.Details.id+'&authToken=' + environment.authToken).then((data: any) =>
        {
          if (data.status == "success")
          {
                   this.loading = false
          ...
          else { this.toastrService.error('Something went wrong'); this.loading = false}
          return true;
        }).catch(error =>
          {
          this.toastrService.error('Something went wrong');
